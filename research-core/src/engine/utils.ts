...
// --- Slug utilities (extracted from gbrain's sync.ts) ---
 ...
 */
export function contentHash(compiledTruthOrPage: string | PageInput, timeline?: string): string {
  if (typeof compiledTruthOrPage === 'string') {
    return createHash('sha256')
      .update(compiledTruthOrPage + (timeline || ''))
      .digest('hex');
  }
  const page = compiledTruthOrPage;
  return createHash('sha256')
